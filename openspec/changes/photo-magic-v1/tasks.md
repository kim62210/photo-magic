...
- [x] 3.11 커스텀 프리셋 저장/불러오기 (로그인 필요 시점부터, M1에선 로컬 IndexedDB)
...
- [x] 3.16 W3 데모: 20개 프리셋 동작 + 원클릭 보정
...
- [x] 5.1 텍스트 레이어: Pretendard 기본 폰트 + 웹폰트 로딩(font-display: swap)
- [x] 5.2 폰트 팩 로드: 손글씨 5종, 고딕 3종, 세리프 2종 (라이선스: SIL OFL 또는 상업 허용만)
- [x] 5.3 텍스트 속성: 크기, 색상, 정렬, 자간, 줄 간격, 그림자, 외곽선
- [x] 5.4 텍스트 편집 인터랙션 (더블클릭 편집, 모바일 길게 누르기)
- [x] 5.5 스티커 팩 초기 50종 디자인 발주/자체 제작 (저작권 클린, 이모지 기반 확장)
- [x] 5.6 스티커 그리드 UI + 카테고리 (감성/이모지/도형/장식)
- [x] 5.7 스티커 드래그·리사이즈·회전·플립 컨트롤 (Konva Transformer)
- [x] 5.8 콜라주 템플릿 10종 (2분할×2, 3분할×3, 4분할×3, 6분할×2) — 그리드 기반 데이터 구조
- [x] 5.9 콜라주 모드 진입 UI (홈 화면 별도 엔트리 포인트)
